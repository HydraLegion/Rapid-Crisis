...
import { PROPERTIES, MOCK_INCIDENTS, ANALYTICS_DATA, SEVERITY_LABELS, STAFF_MEMBERS, PLAYBOOKS, RBAC_MATRIX, SLA_MATRIX } from '../../src/data/mockData'
import { useToast } from '../../components/shared/ToastProvider'

function timeAgo(date) {
  if (!date) return '—'
  const ms = typeof date.getTime === 'function' ? date.getTime() : date
  ...
  return (
    <div style={{ padding: 'var(--space-5)', overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: 'var(--space-5)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 'var(--space-4)' }}>
        {/* Response Time Trend */}
        <div className="glass-card" style={{ padding: 'var(--space-4)', minWidth: 0 }}>
        ...
        <div className="glass-card" style={{ padding: 'var(--space-4)', minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-4)', alignItems: 'center' }}>
            <div style={{ width: '50%', height: 160, position: 'relative' }}>
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
...
function PostmortemBuilder({ addToast }) {
  const [incident] = useState(MOCK_INCIDENTS[3])
...
// ---- MAIN COMMAND CENTER ----
function CommandCenter({ onBack, addToast }) {
...
export default function CommandCenterPage() {
  ...
  return (
    <>
      <Head>
        <title>Command Center — Rapid Crisis</title>
      </Head>
      <CommandCenter 
        onBack={() => router.push('/')} 
        addToast={toast}
